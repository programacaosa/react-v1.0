...
const Header = () => {
  ...
  // Array de cidades com subcidades
  const citiesWithSubCities = [
    ...
    // Adicione mais cidades com suas subcidades conforme necessário
  ];

  // Estado para controlar a cidade sobre a qual o mouse está passando
  ...
  return (
    <div className="desktop-content" style={{ display: 'flex', marginBottom: '20px' }}>
        ...
        <ul style={{ listStyleType: 'none', padding: 0 }}>
          {citiesWithSubCities.map((city) => (
            ...
            >
              <Link
                to={city.link}
                style={{ textDecoration: 'none', color: '#5a5a5a', display: 'block', padding: '15px 0', fontWeight: 'bold', fontSize: '14px' }}
              >
                {city.name}
              </Link>
              {/* Exibição das subcidades ao passar o mouse */}
      ...
      <div style={{ flex: '1', display: 'flex', flexDirection: 'column' }}>
        {/* Conteúdo adicional do lado direito do menu */}
  ...
  const MobileFooter = () => (
    // Componente Footer para Telas Menores (Mobile)
        ...
        <div className="mobile-menu" style={{ backgroundColor: '#202024', padding: '20px', borderRadius: '10px', color: '#ffffff', marginTop: '20px' }}>
          ...
          <div style={{ marginBottom: '10px' }}>
            ...
            <Link to="/menu-item-2" style={{ display: 'block' }}>
              <img src="travesti.jpg" alt="Menu Item 2" style={{ width: '100%', borderRadius: '5px' }} />
            </Link>
          </div>
          <div style={{ marginBottom: '10px' }}>
            <p style={{ color: '#ffffff', fontWeight: 'bold', fontSize: '16px', marginBottom: '5px' }}>CASAS DE MASSAGEM</p>
            <Link to="/menu-item-3" style={{ display: 'block' }}>
              <img src="casa.jpeg" alt="Menu Item 3" style={{ width: '100%', borderRadius: '5px' }} />
            </Link>
          </div>
        </div>
      )}
